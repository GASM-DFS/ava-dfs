 ...
 *
 * --file must be an absolute path to a JSON array of player rows.
...
const { addProbabilisticProjections } = require('../services/inference/projections');
...
Required:
  ...
  --file <absolute-path>    Absolute path to player data JSON file
...
function main() {
  ...
  if (!args.file)                  die('--file <absolute-path> is required');
  if (!path.isAbsolute(args.file)) die('--file must be an absolute path (no auto-discovery)');
